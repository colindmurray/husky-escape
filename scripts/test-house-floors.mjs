...
try {
    ...
    const checks = await page.evaluate(async () => {
        ...
        check(w.props.filter(p => p instanceof HomeDog).map(p => p.quest.dog).join(',') === 'Opal', 'Opal lives in the master bedroom after the chase');
    ...
    await page.getByRole('button', { name: 'Second floor · Master bedroom' }).click();
    ...
    await page.getByRole('button', { name: 'Talk to Opal' }).click();
    ...
    await page.evaluate(() => { const w=window.__husky.engine.world; w.player.x=830; w.player.y=w.height-140; w.update(); });
