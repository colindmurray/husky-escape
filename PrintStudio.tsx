...
const STORIES: [CutsceneType, string][] = [['intro', 'Onyx’s story'], ['pound_escape', 'Escape from the pound'], ['chase', 'The chase'], ['underwater_intro', 'Into the ocean'], ['pier_intro', 'The stormy pier'], ['neon_intro', 'City lights'], ['bakery_intro', 'The warm bakery'], ['town_intro', 'Market day'], ['raccoon_intro', 'Baron von Bins reveal']];
...
export default function PrintStudio() {
    ...
    const [floor, setFloor] = useState<HomeFloor>(['ground', 'basement', 'upstairs'].includes(params.get('floor') || '') ? params.get('floor') as HomeFloor : 'ground');
    ...
    async function generate() {
        ...
        try {
            ...
            if (subject === 'level') {
                ...
                meta.level = ({ intro: 1, pound_escape: 3, chase: 7, underwater_intro: 8, pier_intro: 9, neon_intro: 11, bakery_intro: 12, town_intro: 13, raccoon_intro: 14 })[story];
    ...
    return <main className="studio">
        ...
        <form onSubmit={e => { e.preventDefault(); void generate(); }}>
            ...
            {subject === 'level' ? <>
                ...
                {level === 15 && <label>House floor<select aria-label="House floor" value={floor} onChange={e => change(() => setFloor(e.target.value as HomeFloor))}><option value="ground">Ground floor</option><option value="basement">Basement</option><option value="upstairs">Second floor</option></select></label>}
